...
export const getOrderByNo = async (order_no) => {
  const res = await axios.get(`${API}/packing/order/${order_no}`);
  return res.data;
};

/* ── Validate bobbin for packing ── */
export const validateBobbinForPacking = async (bobbin_no, order_no) => {
  const res = await axios.get(`${API}/packing/validate/${bobbin_no}?order_no=${order_no}`);
  return res.data;
};

/* ── Submit packing list ── */
export const submitPackingList = async (payload) => {
  const res = await axios.post(`${API}/packing/submit`, payload, { headers: authHeaders() });
  return res.data;
};

/* ── Get all packing lists (history) ── */
export const getAllPackingLists = async () => {
  const res = await axios.get(`${API}/packing/list`, { headers: authHeaders() });
  return res.data;
};

/* ── View packing list detail (header + bobbins) ── */
export const viewPackingList = async (orderNo) => {
  const res = await axios.get(`${API}/packing/view/${orderNo}`, { headers: authHeaders() });
  return res.data;
};

/* ── Remove bobbin from packing list ── */
export const removeBobbinFromPacking = async (packingOrderBobbinId) => {
  const res = await axios.delete(`${API}/packing/bobbin/${packingOrderBobbinId}`, { headers: authHeaders() });
  return res.data;
};

/* ── Remove entire box from packing list ── */
export const removeBoxFromPacking = async (orderNo, stackNo, boxNo) => {
  const res = await axios.delete(`${API}/packing/box/${orderNo}/${stackNo}/${boxNo}`, { headers: authHeaders() });
  return res.data;
};

/* ── Add bobbin to existing packing list ── */
export const addBobbinToPacking = async (payload) => {
  const res = await axios.post(`${API}/packing/add-bobbin`, payload, { headers: authHeaders() });
  return res.data;
};
