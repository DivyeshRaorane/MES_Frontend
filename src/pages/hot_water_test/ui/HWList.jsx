...
import { Plus, Search, Eye, Edit2, Waves } from 'lucide-react';
import { showError } from '../../../utils/toastService';
import { getHwEntries } from '../services/hwEntryService';
import HWForm from './HWForm';
import HWView from './HWView';
...
const HWList = () => {
  ...
  const fetchEntries = async () => {
    ...
    try {
      ...
      const res = await getHwEntries(filters);
  ...
  if (viewId) return <HWView entryId={viewId} onBack={() => setViewId(null)} />;
  ...
  return (
    <div className="h-full bg-slate-50 font-sans text-slate-800 flex flex-col overflow-hidden">
      <div className="flex flex-col flex-1 bg-white rounded-xl shadow border border-slate-200 overflow-hidden m-2">
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-200 bg-slate-50/60 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold text-slate-700 uppercase tracking-wider">Hot Water Test Entries</span>
          ...
          <button onClick={() => { setEditId(null); setShowForm(true); }}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white text-[10px] font-bold rounded-lg hover:bg-blue-700 transition-all shadow-sm">
        ...
        <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-100 flex-shrink-0">
          ...
          <button onClick={fetchEntries} className="flex items-center gap-1 px-3 py-1.5 bg-blue-600 text-white text-[9px] font-bold rounded hover:bg-blue-700">
          ...
          {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {entries.map((e, i) => (
                  <tr key={e.hot_water_entry_id} className="hover:bg-blue-50/30">
                    ...
                    <td className="px-3 py-2">
                      <div className="flex gap-1">
                        <button onClick={() => { setEditId(e.hot_water_entry_id); setShowForm(true); }} className="px-2 py-1 bg-blue-50 text-blue-700 text-[8px] font-bold rounded hover:bg-blue-100"><Edit2 size={9} className="inline mr-0.5" />Edit</button>
                        <button onClick={() => setViewId(e.hot_water_entry_id)} className="px-2 py-1 bg-indigo-50 text-indigo-700 text-[8px] font-bold rounded hover:bg-indigo-100"><Eye size={9} className="inline mr-0.5" />View</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {showForm && <HWForm entryId={editId} onClose={() => { setShowForm(false); setEditId(null); fetchEntries(); }} />}
    </div>
  );
};
