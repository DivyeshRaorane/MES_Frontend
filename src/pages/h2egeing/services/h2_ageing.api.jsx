...
export const validateBobbinForH2 = async (bobbin_no, d2_batch_id) => {
  ...
  const res = await axios.get(`${API}/api/h2ageing/validate-bobbin/${bobbin_no}${params}`);
  return res.data;
};

/* ── Submit H2 Issue ── */
export const submitH2Issue = async (payload) => {
  const res = await axios.post(`${API}/api/h2ageing/issue`, payload, { headers: authHeaders() });
  return res.data;
};
...
export const getPendingBeforeBatches = async () => {
  const res = await axios.get(`${API}/api/h2ageing/pending-before`);
  return res.data;
};

/* ── Pending batches for After Entry ── */
export const getPendingAfterBatches = async () => {
  const res = await axios.get(`${API}/api/h2ageing/pending-after`);
  return res.data;
};

/* ── Pending batches for 14 Days Entry ── */
export const getPending14DayBatches = async () => {
  const res = await axios.get(`${API}/api/h2ageing/pending-14day`);
  return res.data;
};

/* ── Get bobbins for a specific H2 batch ── */
export const getBobbinsForBatch = async (h2_batch_id) => {
  const res = await axios.get(`${API}/api/h2ageing/bobbins/${h2_batch_id}`);
  return res.data;
};

/* ── Save Before Entry ── */
export const saveBeforeEntry = async (payload) => {
  const res = await axios.put(`${API}/api/h2ageing/before-entry`, payload, { headers: authHeaders() });
  return res.data;
};

/* ── Save After Entry ── */
export const saveAfterEntry = async (payload) => {
  const res = await axios.put(`${API}/api/h2ageing/after-entry`, payload, { headers: authHeaders() });
  return res.data;
};

/* ── Save 14 Day Entry ── */
export const save14DayEntry = async (payload) => {
  const res = await axios.put(`${API}/api/h2ageing/14day-entry`, payload, { headers: authHeaders() });
  return res.data;
};
