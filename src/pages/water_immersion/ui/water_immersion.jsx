...
import { Droplet, Activity, Plus, Minus, Scan } from 'lucide-react';
...
import { fetchWiByBarcode, saveWiEntry } from '../services/water_immersion.api';
...
const makeRow = () => ({ wi_date: '', day: '', at_1310: '', at_1550: '', at_1625: '' });
...
/* ══════════════════════════════════════════════════════════ */
const WaterImmersion = () => {
  const [loading, setLoading] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [fetched, setFetched] = useState(false);
  const [existingEntryId, setExistingEntryId] = useState(null);

  /* ── Fetch bobbin info + existing data ── */
  const handleBarcodeFetch = async (barcode, setValues) => {
    ...
    try {
      const res = await fetchWiByBarcode(val);
      ...
      if (res?.wi_entry) {
        const te = res.wi_entry;
        setExistingEntryId(te.wi_entry_id);
        ...
        showWarning('Existing Water Immersion entry found. Data loaded.');
      }

      if (res?.days && res.days.length > 0) {
        newVals.rows = res.days.map(r => ({
          wi_date: r.wi_date ? r.wi_date.split('T')[0] : '',
          day: r.wi_day ?? '',
          at_1310: r.at_1310 ?? '',
          at_1550: r.at_1550 ?? '',
          at_1625: r.at_1625 ?? '',
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      const rows = values.rows.map(row => ({
        wi_date: row.wi_date || null,
        wi_day: row.day ? Number(row.day) : null,
        at_1310: row.at_1310 ? Number(row.at_1310) : null,
      ...
      const res = await saveWiEntry(payload);
      if (res?.success) {
        showSuccess(existingEntryId ? 'Water Immersion entry updated' : 'Water Immersion entry saved');
            ...
            <Form className="flex flex-col flex-1 overflow-hidden px-3 py-1.5 gap-1.5">
              ...
              <div className="flex-shrink-0 bg-white rounded-lg border border-slate-200 shadow-sm px-3 py-2">
                <div className="flex items-center gap-2 mb-1.5">
                  <Droplet size={12} className="text-cyan-600" />
                  <span className="text-[9px] font-bold text-slate-700 uppercase tracking-wider">Water Immersion Test</span>
                  {existingEntryId && (
                        ...
                        <table className="w-full text-left border-collapse">
                          <thead className="sticky top-0 z-20">
                            <tr className="bg-slate-800 text-white">
                              ...
                              <th className="px-3 py-2 text-[9px] font-bold uppercase border-r border-slate-600 text-center bg-cyan-700">AT 1310 (dB)</th>
                              <th className="px-3 py-2 text-[9px] font-bold uppercase border-r border-slate-600 text-center bg-cyan-700">AT 1550 (dB)</th>
                              <th className="px-3 py-2 text-[9px] font-bold uppercase text-center bg-cyan-700">AT 1625 (dB)</th>
                            </tr>
                          </thead>
                          <tbody>
                            {form.values.rows.map((_, idx) => (
                              <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-1 py-1 border-r border-slate-100">
                                  <TC name={`rows.${idx}.wi_date`} type="date" w="w-full" />
                                </td>
                                <td className="px-1 py-1 border-r border-slate-100">
                                  <TC name={`rows.${idx}.day`} type="number" placeholder="1" w="w-full" />
                                </td>
                                <td className="px-1 py-1 border-r border-slate-100 bg-cyan-50/20">
                                  <TC name={`rows.${idx}.at_1310`} placeholder="0.000" w="w-full" />
                                </td>
                                <td className="px-1 py-1 border-r border-slate-100 bg-cyan-50/20">
                                  <TC name={`rows.${idx}.at_1550`} placeholder="0.000" w="w-full" />
                                </td>
                                <td className="px-1 py-1 bg-cyan-50/20">
