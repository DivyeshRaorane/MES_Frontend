...
import { Plus, Search, Eye, Edit2, Thermometer } from 'lucide-react';
import { showError } from '../../../utils/toastService';
import { getTempEntries } from '../services/tempEntryService';
...
const TemperatureList = () => {
  ...
  const fetchEntries = async () => {
    ...
    try {
      ...
      const res = await getTempEntries(filters);
  ...
  if (viewId) return <TemperatureView entryId={viewId} onBack={handleViewClose} />;
  ...
  return (
    <div className="h-full bg-slate-50 font-sans text-slate-800 flex flex-col overflow-hidden">
      <div className="flex flex-col flex-1 bg-white rounded-xl shadow border border-slate-200 overflow-hidden m-2">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-200 bg-slate-50/60 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold text-slate-700 uppercase tracking-wider">Temperature Entries</span>
            <span className="text-[9px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold">{entries.length}</span>
          </div>
        ...
        <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-100 flex-shrink-0">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search Bobbin No..."
            ...
            className="border border-slate-200 rounded px-2 py-1.5 text-[10px] outline-none focus:ring-1 focus:ring-blue-300 w-40" />
          <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
            className="border border-slate-200 rounded px-2 py-1.5 text-[10px] outline-none w-28" />
          <span className="text-[9px] text-slate-400">to</span>
          <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)}
            className="border border-slate-200 rounded px-2 py-1.5 text-[10px] outline-none w-28" />
          <button onClick={handleSearch}
            className="flex items-center gap-1 px-3 py-1.5 bg-blue-600 text-white text-[9px] font-bold rounded hover:bg-blue-700">
          ...
          {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 bg-slate-800 text-white z-10">
                <tr>
                  {['#', 'Bobbin No', 'Fiber Length', 'Result', 'Prepared By', 'Checked By', 'Start Date', 'End Date', 'Created', 'Actions'].map(h => (
              ...
              <tbody className="divide-y divide-slate-100">
                {entries.map((e, i) => (
                  <tr key={e.temp_entry_id} className="hover:bg-blue-50/30">
                    <td className="px-3 py-2 text-[9px] text-slate-400 font-bold">{i + 1}</td>
                    <td className="px-3 py-2 text-[10px] font-mono font-bold text-blue-700">{e.bobbin_no || '—'}</td>
                    <td className="px-3 py-2 text-[10px] font-mono">{e.fiber_length || '—'}</td>
                    <td className="px-3 py-2">
                      {e.result ? (
                        <span className={`text-[8px] font-bold px-2 py-0.5 rounded-full uppercase ${e.result === 'pass' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>{e.result}</span>
                      ) : <span className="text-[9px] text-slate-400">—</span>}
                    </td>
                    <td className="px-3 py-2 text-[10px] text-slate-600">{e.prepared_by || '—'}</td>
                    ...
                    <td className="px-3 py-2">
                      <div className="flex gap-1">
                        <button onClick={() => handleEdit(e.temp_entry_id)} className="px-2 py-1 bg-blue-50 text-blue-700 text-[8px] font-bold rounded hover:bg-blue-100"><Edit2 size={9} className="inline mr-0.5" />Edit</button>
                        <button onClick={() => handleView(e.temp_entry_id)} className="px-2 py-1 bg-indigo-50 text-indigo-700 text-[8px] font-bold rounded hover:bg-indigo-100"><Eye size={9} className="inline mr-0.5" />View</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {/* Form Modal */}
      {showForm && <TemperatureForm entryId={editId} onClose={handleFormClose} />}
    </div>
  );
};
