...
import { Zap, Activity, Plus, Minus, Scan } from 'lucide-react';
...
import { fetchAatByBarcode, saveAatEntry } from '../services/accelerated_ageing.api';
...
const makeRow = () => ({ aat_date: '', day: '', at_1310: '', at_1550: '', at_1625: '' });
...
const AcceleratedAgeing = () => {
  ...
  const handleBarcodeFetch = async (barcode, setValues) => {
    ...
    try {
      const res = await fetchAatByBarcode(val);
      ...
      if (res?.aat_entry) {
        const te = res.aat_entry;
        setExistingEntryId(te.aat_entry_id);
        ...
        showWarning('Existing Accelerated Ageing entry found. Data loaded.');
      }

      if (res?.days && res.days.length > 0) {
        newVals.rows = res.days.map(r => ({
          aat_date: r.aat_date ? r.aat_date.split('T')[0] : '',
          day: r.aat_day ?? '',
          at_1310: r.at_1310 ?? '',
          at_1550: r.at_1550 ?? '',
          at_1625: r.at_1625 ?? '',
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      const rows = values.rows.map(row => ({
        aat_date: row.aat_date || null,
        aat_day: row.day ? Number(row.day) : null,
      ...
      const res = await saveAatEntry(payload);
      if (res?.success) {
        showSuccess(existingEntryId ? 'Accelerated Ageing entry updated' : 'Accelerated Ageing entry saved');
            ...
            <Form className="flex flex-col flex-1 overflow-hidden px-3 py-1.5 gap-1.5">
              ...
              <div className="flex-shrink-0 bg-white rounded-lg border border-slate-200 shadow-sm px-3 py-2">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-[9px] font-bold text-slate-700 uppercase tracking-wider">Accelerated Ageing Test</span>
                  {existingEntryId && (
                        ...
                        <table className="w-full text-left border-collapse">
                          <thead className="sticky top-0 z-20">
                            <tr className="bg-slate-800 text-white">
                              ...
                              <th className="px-3 py-2 text-[9px] font-bold uppercase border-r border-slate-600 text-center bg-amber-700">AT 1310 (dB)</th>
                              <th className="px-3 py-2 text-[9px] font-bold uppercase border-r border-slate-600 text-center bg-amber-700">AT 1550 (dB)</th>
                              <th className="px-3 py-2 text-[9px] font-bold uppercase text-center bg-amber-700">AT 1625 (dB)</th>
                            </tr>
                          </thead>
                          <tbody>
...
export default AcceleratedAgeing;
