...
import { Plus, Search, Eye, Edit2, Flame } from 'lucide-react';
import { showError } from '../../../utils/toastService';
import { getHthaEntries } from '../services/hthaEntryService';
import HTHAForm from './HTHAForm';
import HTHAView from './HTHAView';
...
const HTHAList = () => {
  ...
  const fetchEntries = async () => {
    ...
    try {
      ...
      const res = await getHthaEntries(filters);
  ...
  if (viewId) return <HTHAView entryId={viewId} onBack={() => setViewId(null)} />;
  ...
  return (
    <div className="h-full bg-slate-50 font-sans text-slate-800 flex flex-col overflow-hidden">
      <div className="flex flex-col flex-1 bg-white rounded-xl shadow border border-slate-200 overflow-hidden m-2">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-200 bg-slate-50/60 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-bold text-slate-700 uppercase tracking-wider">HTHA Entries</span>
            <span className="text-[9px] bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full font-bold">{entries.length}</span>
          </div>
          <button onClick={() => { setEditId(null); setShowForm(true); }}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-600 text-white text-[10px] font-bold rounded-lg hover:bg-orange-700 transition-all shadow-sm">
        ...
        <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-100 flex-shrink-0">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search Bobbin No..."
            ...
            className="border border-slate-200 rounded px-2 py-1.5 text-[10px] outline-none focus:ring-1 focus:ring-orange-300 w-40" />
          ...
          <button onClick={fetchEntries} className="flex items-center gap-1 px-3 py-1.5 bg-orange-600 text-white text-[9px] font-bold rounded hover:bg-orange-700">
          ...
          {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {entries.map((e, i) => (
                  <tr key={e.htha_entry_id} className="hover:bg-orange-50/30">
                    <td className="px-3 py-2 text-[9px] text-slate-400 font-bold">{i + 1}</td>
                    <td className="px-3 py-2 text-[10px] font-mono font-bold text-orange-700">{e.bobbin_no || '—'}</td>
                    ...
                    <td className="px-3 py-2">
                      <div className="flex gap-1">
                        <button onClick={() => { setEditId(e.htha_entry_id); setShowForm(true); }} className="px-2 py-1 bg-blue-50 text-blue-700 text-[8px] font-bold rounded hover:bg-blue-100"><Edit2 size={9} className="inline mr-0.5" />Edit</button>
                        <button onClick={() => setViewId(e.htha_entry_id)} className="px-2 py-1 bg-orange-50 text-orange-700 text-[8px] font-bold rounded hover:bg-orange-100"><Eye size={9} className="inline mr-0.5" />View</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {showForm && <HTHAForm entryId={editId} onClose={() => { setShowForm(false); setEditId(null); fetchEntries(); }} />}
    </div>
  );
};

export default HTHAList;
